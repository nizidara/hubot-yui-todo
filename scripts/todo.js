...
module.exports = (robot) => {
	...
	robot.hear(/今日のアニメ/i, (msg) =>{
		const list = todo.list_anime_today(today_day);
		const username = msg.message.user.name;
	...
	robot.hear(/昨日のアニメ/i, (msg) =>{
		const list = todo.list_anime_today(yesterday_day);
		const username = msg.message.user.name;
	...
	robot.hear(/アニメ (.+)/i, (msg) =>{
		const day = msg.match[1].trim();
		const list = todo.list_anime_today(day);
		const username = msg.message.user.name;
	...
	robot.hear(/返金 (.+)/i, (msg) => {
		const name = msg.match[1].trim();
		const namedel = todo.del_money(name);
		const username = msg.message.user.name;
	...
	robot.hear(/金額 (.+)/i, (msg) => {
		const name = msg.match[1].trim();
		const much = todo.search_money(name);
		const username = msg.message.user.name;
